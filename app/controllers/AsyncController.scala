...
@Singleton
class TestController @Inject()(@NamedDatabase("abhilash") dbConfigProvider: DatabaseConfigProvider) extends Controller {

  def index = Action.async({ implicit request =>  Future {
    Ok
  }})
}
